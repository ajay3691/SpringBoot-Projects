...
public class TaskServiceImpl implements TaskService {
	...
	public TaskDto saveTask(long userid, TaskDto taskDto) {
		...
		Users user = userRepository.findById(userid).get();    // get();  message.. user not found 
	...
	public List<TaskDto> getAllTasks(long usersid) {
		// TODO Auto-generated method stub
		userRepository.findById(usersid).get(); /* // get(); // message.. user not found
				.orElseThrow(() -> new UserNotFound(String.format("User Id %d not found", usersid)));*/
	...
	public TaskDto getTask(long userid, long taskid) {
		// TODO Auto-generated method stub
		Users users = userRepository.findById(userid).get(); // get(); // message.. user not found
				/*.orElseThrow(() -> new UserNotFound(String.format("User Id %d not found", userid)));*/	
		Task task = taskRepository.findById(taskid).get();    // get(); // message.. user not found 
				/*.orElseThrow(() -> new TaskNotFound(String.format("Task Id %d not found", taskid)));*/
		if (users.getId() != task.getUsers().getId());/* {     // get(); /* message.. user not found 
